...
3. Generate `conftest.py` with a fixture that reads the source database path
   from `config.settings` (`DATABASE_PATH`), copies it into a temporary file
   and enables foreign keys. Do not scan directories with glob. Never modify
   the source database.
...
8. Output the generated files as `GeneratedTestSuite` JSON.
